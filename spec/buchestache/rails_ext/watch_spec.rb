...
describe "Buchestache::Watch" do
  it "adds the #watch method to Buchestache" do
    expect(Buchestache.instance).to respond_to(:watch)
  end

  describe "#watch" do
    it "can subscribe to any notification" do
      @dummy = Object.new.stub(:foo)
      @subscriber = Buchestache.instance.watch('foo.bar') { @dummy.foo }
    ...
    it "is provided with a handy named store" do
      Buchestache.instance.watch('foo.bar') { |*event, store| store[:foo] = 'bar' }
      @subscriber = ActiveSupport::Notifications.instrument('foo.bar')
      expect(Buchestache.instance.store['foo.bar']).to eq(foo: 'bar')
    end
  end
end
